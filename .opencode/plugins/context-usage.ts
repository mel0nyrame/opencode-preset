...
async function importFromVendor(pkg: string) {
  ...
  try {
    ...
    throw new Error(
      "Context usage dependencies missing. Run the install.sh script in the plugins directory to install vendor tokenizers.",
    )
  }
...
function formatTokenizerResolutionError(error: TokenizerResolutionError, sessionID: string): string {
  ...
  lines.push(
    "Install or update vendor tokenizers by running the install.sh script in the plugins directory.",
  )
